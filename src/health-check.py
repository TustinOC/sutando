...
def _voice_log_path() -> Path:
    ...
    - launchd plist (~/Library/LaunchAgents/com.sutando.voice-agent.plist)
      ...
      `$SUTANDO_WORKSPACE/logs/voice-agent.log`, in which case the resolver falls
      through to the workspace path below and behavior is unchanged.
    - `src/startup.sh:153` writes to `$SUTANDO_WORKSPACE/logs/voice-agent.log`
...
def check_notes_split_brain() -> "dict | None":
    ...
    <repo>/notes/ and $SUTANDO_WORKSPACE/notes/ — fires only when the two paths differ."""
    ...
    return {
        ...
        "status": "warn",
        "detail": (
            f"{len(overlap)} .md file(s) duplicated across <repo>/notes/ and $SUTANDO_WORKSPACE/notes/ "
...
def run_all_checks() -> list[dict]:
    ...
    # Notes split-brain: both <repo>/notes/ and $SUTANDO_WORKSPACE/notes/ with overlapping files (#1266)
...
def _any_core_alive(workspace: Optional[Path] = None, max_age_s: float = 90.0) -> bool:
    ...
    Each running core writes `$SUTANDO_WORKSPACE/state/cores/<hostname>.alive` every
