		...
		if (concatMatch) {
			try {
				...
				if (image && video) {
					...
					const claudeCfgDir = process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.claude');
					const scriptPath = resolve(join(claudeCfgDir, 'skills/video-concat/scripts/prepend-image.sh'));
