...
3. **Discord** — Read recent messages from `$SUTANDO_WORKSPACE/logs/discord-bridge.log` (tail ~100 lines). Summarize anything actionable from overnight. Reference channel ID mapping at `$SUTANDO_MEMORY_DIR/reference_discord_channels.md`. Only surface messages NOT already replied to by the bridge.

4. **Pending tasks** — Check `$SUTANDO_WORKSPACE/pending-questions.md` for unanswered items. Check `$SUTANDO_WORKSPACE/tasks/` for queued tasks.
