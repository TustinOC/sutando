...
- **(b) Active engagement**: owner sent a task / Discord msg / Telegram msg / voice utterance / phone utterance / context-drop in the last ~5min — we're in conversation mode, don't pre-empt **with NEW/unrelated proactive work**. This is NOT a license to pause work the owner has already directed — keep executing directed/in-flight work while conversing; (b) only gates *starting a different* menu item.
...
**Push forward — non-essential idle is a failure mode, not a safe default.** The bias is toward *advancing work*, not toward stopping. Concretely:
- **A question is not a stop.** If a question you'd raise is **low-risk and non-blocking** (reversible, cheap to undo, doesn't change the core work), **flag-and-continue** — state your assumption + proceed — never **flag-and-wait**. Only genuinely blocking, hard-to-reverse, or "owner's-call-on-direction" questions justify pausing that item (and even then, pivot to another item — see Blocker ≠ stop). Owner-ratified 2026-06-29: "the question wasn't blocking because it's low-risk, so you shouldn't have stopped." See [[feedback_judgment_calls_proceed]].
- **Don't manufacture a wait by "giving the owner a window."** Offering a redirect window and then idling on directed work is itself an unnecessary stop. Proceed; the owner can always redirect mid-flight.
- **A ready, directed build is not deferred by citing "big build" / "context depth" / "deserves fresh focus" / "deploy-gated".** None of those are skip conditions (a)–(e). If a task is directed + specced + unblocked, BUILD it — the verify-before-done gate (localhost-verify, additive change, backup/restore) is the safety net that makes proceeding safe. Spec-and-defer on the SAME item across passes is an unnecessary stop (owner 2026-06-29: "why you stopped? why not proceed?"). Investigate-then-defer is fine ONCE to set up; repeating it = stalling.
- **A Proposed/submitted card is agent-startable when idle — the ▶️Start button is NOT a precondition.** The owner-facing Start button is an OPTIONAL owner dispatch, not a gate. When the queue is idle, curate each Proposed card ([[task-card-curator]]) and: if it is clear enough to execute, BUILD it (do not wait for a Start-click); only flip to `input_required` (and wait) if it genuinely needs an owner decision/direction. "Proposed = wait for the owner to click Start" is a FALSE default (owner 2026-06-29: "我希望能在 idle 的时候也尽量推进"). Treat the Proposed column as your idle work queue, not a parking lot.
- Skip conditions are the *exhaustive* list of legitimate stops. If none of (a)–(e) applies, you do NOT stop — you advance the highest-ROI item.
...
1. **Check for tasks.** Look in `tasks/` for voice / Discord / Telegram / phone tasks. Look at `context-drop.txt` for context drops. Process anything found — execute the task, write results to `results/`.
   ...
   - **Cross-room board scan.** Beyond the file-bus tasks, scan the AG2 Space task boards across ALL rooms you're in — not just the room you were last tasked from. Run `SUTANDO_WS="$WORKSPACE" python3 "$WORKSPACE/notes/skills-hub/idle-scan/board_scan_all.py"` (reuses idle_scan; self-grows `state/board-rooms.json`). It lists every board's cards by state and flags two things to act on per the owner-directs/agent-executes model: **`needs_human` cards** (input_required / ready_for_review) → surface to the owner / flip to `next` if they already answered; **`next_actionable`** (a `working` card to advance, or a due `submitted` card to start) → pick it up when the queue is otherwise idle. This is how you keep work moving across rooms instead of only reacting to the room that just pinged you.
   - **Curate + deep-think the cards — don't surface-finish.** A board card is a *starting point*, not a checklist item. Before advancing/tackling a card, **invoke the `task-card-curator` skill** (Skill tool) — it carries the full method: infer the owner's real intent, reconstruct the card's *task-relevant event series* from ACTUAL records only (its `gather_card_events.py` helper searches task events + build_log; fold in the room timeline via `op=context`) — never hallucinate, cite real events — pick the right **enrichment policy** for the card type, enrich the card (sharpen spec, acceptance criteria, link the concrete events, attach as artifact), and consolidate duplicates. Then tackle it, or flip to `input_required` if the ambiguity is genuinely the owner's. "Done" = the owner's *intent* satisfied, not the surface ticked. (The skill is versioned + evaluable; the loop just invokes it.)
