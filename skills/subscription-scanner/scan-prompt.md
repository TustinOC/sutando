...
If the scan finds anything `added` or `removed` since the previous run, write a `proactive-{ts}.txt` to `$SUTANDO_WORKSPACE/results/` with a brief summary (not the full table — just "+1 added: X. -1 removed: Y") so the change is surfaced via Telegram.
