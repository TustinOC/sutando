...
| `/tmp/pointer-cmd.json` file poll | **`$SUTANDO_WORKSPACE/state/pointer-cmd.json`**, watched with the app's existing `DispatchSource` dir-watch idiom (same as `watchResults()`) — resolves open item #4 | ✅ done |
