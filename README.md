...
```
    ...
    ↻ = a cron job fires the `/proactive-loop` skill every 5 minutes
        ...
        `$SUTANDO_WORKSPACE/tasks/` via Claude Code's `Monitor` tool — pending
...
Voice agent and conversation server handle conversation-scope actions with **inline tools** — in-process calls that round-trip instantly (describe the screen, hang up, send DTMF, read the clipboard/current time, capture a screenshot). For anything outside that scope they write to `$SUTANDO_WORKSPACE/tasks/`; core reads them, executes, and writes to `$SUTANDO_WORKSPACE/results/`, which each channel speaks or messages back. Telegram and Discord bridges only use the task path.
...
> - **Actions are auditable.** Every Claude Code invocation lands in `$SUTANDO_WORKSPACE/build_log.md`, every task in `$SUTANDO_WORKSPACE/tasks/` + `$SUTANDO_WORKSPACE/results/`, every shell call in the service logs (`$SUTANDO_WORKSPACE/logs/*.log`). Use `tail -f $SUTANDO_WORKSPACE/build_log.md` while it works to watch in real time.
...
| Task delegation (voice → Claude) | `task-bridge.ts` + `watch-tasks-stream.sh` + `$SUTANDO_WORKSPACE/tasks/` | Verified |
| Screen capture + analysis | `macos-tools` skill | Verified |
| Notes / second brain | `$SUTANDO_WORKSPACE/notes/` (YAML-frontmatter markdown) | Verified |
